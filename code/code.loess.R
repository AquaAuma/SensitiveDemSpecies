...
for (i in 1:length(select50)){
  ...
  yearly.s.mean <- yearly.s.mean %>%
    ...
    filter(numh<=10) # remove year when abundance is more than 5 times the long-term average
  ...
  if(length(comb.surveys)>0){
    ...
    names(dat.fit) <- 'fit'
    dat.fit$se.low <- dat.fit$fit-1.96*loess.p.w$se.fit
    dat.fit$se.high <- dat.fit$fit+1.96*loess.p.w$se.fit
    ...
    print(span)
    loess.spp <- loess(numh ~ Year, data=dat.index, span=0.5)
    loess.p <- predict(loess.spp, newdata=seq(from=min(dat.index$Year),to=last.year, by=1), se=TRUE)
    
    dat.fit <- data.frame(loess.p$fit)
    names(dat.fit) <- 'fit'
    dat.fit$se.low <- dat.fit$fit-1.96*loess.p$se.fit
    dat.fit$se.high <- dat.fit$fit+1.96*loess.p$se.fit
    dat.fit$Year <- seq(from=min(dat.index$Year),to=last.year, by=1)
